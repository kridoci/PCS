...
int main() {
	...
	for (int i = 0; i < 100; i++) {
		...
		if (!is_sorted(v)) {
			std::cerr << "Errore: vettore di int non ordinato correttamente con Bubble Sort!\n";
			return EXIT_FAILURE;
		}
	}
	...
	if (!is_sorted(vs)) {
		std::cerr << "Errore: vettore di stringhe non ordinato correttamente con Bubble Sort!\n";
		return EXIT_FAILURE;
	}
